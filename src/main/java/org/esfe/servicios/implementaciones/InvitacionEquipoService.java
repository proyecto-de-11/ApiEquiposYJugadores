...
public class InvitacionEquipoService implements IInvitacionEquipoService {
    ...
    public InvitacionSalidaDto crear(InvitacionCrearDto dto) {
        ...
        // 2. Validación de Unicidad: No permitir más de una invitación PENDIENTE al mismo usuario
        ...
        // 3. Validación de Membresía Existente: El usuario ya es miembro (activo o inactivo)
        ...
        if (miembrosActuales >= equipo.getMaxMiembros()) {
            throw new IllegalStateException("El equipo ya alcanzó su límite máximo de miembros. No se puede enviar la invitación.");
        }
    ...
    public void eliminarPorId(Integer id) {
        ...
        // Se puede añadir lógica de negocio aquí (ej. verificar permisos del usuario que elimina)
    ...
    public InvitacionSalidaDto responderInvitacion(InvitacionResponderDto dto) {
        ...
        // 1. Validar que la invitación esté pendiente y que el usuario respondiendo sea el invitado
        if (invitacion.getEstado() != EstadoInvitacion.PENDIENTE) {
            throw new IllegalStateException("La invitación ya ha sido " + invitacion.getEstado().getValor().toLowerCase() + ".");
        }
        if (!invitacion.getUsuarioInvitadoId().equals(dto.getUsuarioRespondioId())) {
            throw new SecurityException("No tiene permiso para responder esta invitación.");
        }

        // 2. Procesar la respuesta ACEPTADA
        if (dto.getNuevoEstado() == EstadoInvitacion.ACEPTADA) {
            
            // a. VALIDACIÓN CLAVE: Verificar si el equipo está lleno
            Equipo equipo = invitacion.getEquipo(); // El equipo ya está cargado (posiblemente de forma Lazy)
            ...
            if (miembrosActivos >= equipo.getMaxMiembros()) {
                // Si el equipo está lleno, marcamos la invitación como RECHAZADA por capacidad
                invitacion.setEstado(EstadoInvitacion.RECHAZADA);
                // O lanzar excepción para notificar al usuario (más recomendado)
                throw new IllegalStateException("El equipo " + equipo.getNombre() + " ya está lleno. La invitación no puede ser aceptada.");
            }
        ...
        } else if (dto.getNuevoEstado() == EstadoInvitacion.RECHAZADA || dto.getNuevoEstado() == EstadoInvitacion.CANCELADA) {
            // 3. Procesar Rechazo o Cancelación
    ...
    public Page<InvitacionSalidaDto> obtenerPaginadoYFiltrado(Optional<String> busquedaMensaje, Pageable pageable) {
        ...
        if (busquedaMensaje.isPresent() && !busquedaMensaje.get().isEmpty()) {
            ...
            // Obtener todos, ordenados por ID descendente por defecto (más reciente primero)
